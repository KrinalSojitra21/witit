...
const circleFeedSlice = createSlice({
  name: "circleFeedPost",
  ...
  reducers: {
    ...
    updateCircleCommentCount(state, action: PayloadAction<{ postId: string }>) {
    ...
    updateCircleLikeStatus(state, action: PayloadAction<{ postId: string }>) {
      const { postId } = action.payload;
      const updatedState = state.map((post) =>
        post.postId === postId
          ? {
    ...
    updateCircleRepostStatus(state, action: PayloadAction<{ postId: string }>) {
      const { postId } = action.payload;
      const updatedState = state.map((post) =>
        post.postId === postId
          ? {
    ...
    deleteCirclePost(state, action: PayloadAction<{ postId: string }>) {
      const { postId } = action.payload;
      const updatedState = state.filter((post) => post.postId !== postId);
      return updatedState;
    },
    updateCirclePostView(state, action: PayloadAction<{ postId: string }>) {
      const { postId } = action.payload;
      const updatedState = state.map((post) =>
        post.postId === postId
          ? {
...
export const {
  ...
  updateCirclePostViewPrompt,
} = circleFeedSlice.actions;
export default circleFeedSlice.reducer;
